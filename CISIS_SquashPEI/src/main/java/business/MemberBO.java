...
public class MemberBO {
    ...
    public static void changePassword(Member theMember, ChangePassword changePassword) throws PasswordException {
        ...
        //if we are changing someone else's password dont check this...
        if (!(theMember.getPassword() == null)) {
    ...
    public static void addMember(Member member) {
        try {
            ...
            while(MemberDAO.checkUserid(username) == false){
                System.out.println("###UserName does already exist ");
                userExists += 1;
                username += userExists;
                member.setNewUserId(username);
            }
            if(MemberDAO.checkUserid(username) == false) {
                System.out.println("###UserName doesnt already exist ");
                member.setNewUserId(username);
